...
def ARPA_calculations(objectA, objectB, *args, **kwargs):
    ...
    if ((objectA_speed == objectB_speed) and (vectorA_angle == vectorB_angle)) or (
        ...
        if d_map == True:
            ...
            url = get_gmap_url(pointA, objectA_speed, vectorA_angle, pointB, objectB_speed, vectorB_angle,
                               relvect=vectorB_angle_relativ, relspeed=objectB_speed_relative)
        ...
        if check_ship_going_away(pointA, vectorA_angle, pointB, vectorB_angle_relativ, objectB_speed_relative) == True:
            ...
            print
            "Ship going away, already at her minimum CPA"
            ...
            if d_map == True and posAatcpa == True and posBatcpa == True:
                url = get_gmap_url(pointA, objectA_speed, vectorA_angle, pointB, objectB_speed, vectorB_angle,
                                   lonAcpa=lonAcpa, latAcpa=latAcpa, lonBcpa=lonBcpa, latBcpa=latBcpa,
                                   relvect=vectorB_angle_relativ, relspeed=objectB_speed_relative)
                return {'cpa': cpa, 'tcpa': tcpa, 'url': url}

            elif d_map == True and posAatcpa == True:
                url = get_gmap_url(pointA, objectA_speed, vectorA_angle, pointB, objectB_speed, vectorB_angle,
                                   lonAcpa=lonAcpa, latAcpa=latAcpa, relvect=vectorB_angle_relativ,
                                   relspeed=objectB_speed_relative)
                return {'cpa': cpa, 'tcpa': tcpa, 'url': url}
...
def check_ship_going_away(pointA, vectorA_angle, pointB, vectorB_angle_relativ, objectB_speed_relative):
    ...
    if calculate_distance(pointA, calculate_future_position(pointB, 0.0001 * objectB_speed_relative,
                                                            vectorB_angle_relativ)) < calculate_distance(pointA,
                                                                                                         pointB):
        return False
    else:
        return True
...
def calculate_cp_position(pointA, objectA_speed, vectorA_angle, pointB, objectB_speed, vectorB_angle,
                          vectorB_angle_relativ, objectB_speed_relative):
...
def calculate_future_position(pointA, object_speed, vector_angle):
    ...
    lon2 = lon1 + atan2(sin(vector_angle) * sin(object_speed / 6378.137) * cos(lat1),
                        cos(object_speed / 6378.137) - sin(lat1) * sin(lat2))
...
def get_gmap_url(pointA, objectA_speed, vectorA_angle, pointB, objectB_speed, vectorB_angle, *args, **kwargs):
    ...
    if relvect != None and relspeed != None:
        ...
        gmap_url += '&path=color:white|weight:2|geodesic:true|' + str(lat2) + ',' + str(lon2) + '|' + str(
            latBrelfut) + ',' + str(lonBrelfut)

        latArel, lonArel = calculate_future_position((latBrelfut, lonBrelfut), objectA_speed, vectorA_angle)
        gmap_url += '&path=color:green|weight:2|geodesic:true|' + str(latBrelfut) + ',' + str(lonBrelfut) + '|' + str(
            latArel) + ',' + str(lonArel)

    gmap_url += '&path=color:green|weight:5|geodesic:true|' + str(lat1) + ',' + str(lon1) + '|' + str(
        latAfut) + ',' + str(lonAfut)
    gmap_url += '&path=color:red|weight:5|geodesic:true|' + str(lat2) + ',' + str(lon2) + '|' + str(
        latBfut) + ',' + str(lonBfut)
